...
class ReedSolomon {
        ...
        for (let i = 0; i < t2; i++) {
            ...
            for (let j = 0; j < poly.length; j++) {
                ...
                newPoly[j + 1] = this.gf.add(
                    newPoly[j + 1],
                    this.gf.mul(poly[j], this.gf.expTable[i]),
                )
            }
            poly = newPoly
        }
    ...
    encode(data) {
        ...
        for (let i = 0; i < this.k; i++) {
            ...
            for (let j = 0; j < remainder.length; j++) {
                remainder[j] = this.gf.sub(
                    remainder[j],
                    this.gf.mul(this.generator[j + 1], coef),
                )
            }
        }
    ...
    _computeSyndromes(received) {
        ...
        for (let i = 0; i < 2 * this.t; i++) {
            ...
            for (let j = 0; j < this.n; j++) {
                s = this.gf.add(
                    this.gf.mul(s, this.gf.expTable[i]),
                    received[j],
                )
            }
            syndromes[i] = s
    ...
    decode(received) {
        if (received.length !== this.n)
            throw new Error(`接收数据长度必须为 ${this.n}`)
