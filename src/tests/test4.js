...
class GF256 {
    ...
    div(a, b) {
        return a && b ? this.exp[(this.log[a] - this.log[b] + 255) % 255] : 0
    }
    pow(a, n) {
...
class ReedSolomon {
    ...
    encode(data) {
        if (data.length > this.k)
            throw new Error(`Data too long (max ${this.k} bytes)`)
        // 补零到k长度
    ...
    _berlekampMassey(s) {
        ...
        for (let n = 0; n < s.length; n++) {
            ...
            if (d === 0) {
                ...
                const temp = [...lam]
                const coef = this.gf.div(
                    d,
                    this.gf.mul(b[0], this.gf.pow(this.gf.exp[1], m - 1)) || 1,
                )
                // 更新 lambda
    ...
    _forney(lam, s, pos) {
        ...
        for (let i = 0; i < lamPrime.length; i++) {
            denom = this.gf.add(
                denom,
                this.gf.mul(lamPrime[i], this.gf.pow(x, i)),
            )
        }
        if (denom === 0) return 0
    ...
    decode(rx) {
        if (rx.length !== this.n)
            throw new Error(`Received data must be ${this.n} bytes`)
...
;(function test() {
    try {
        ...
        console.log(
            '解码后数据：',
            Array.from(decoded.slice(0, original.length)),
        )

        // 6. 验证
        const isOK =
            JSON.stringify(Array.from(original)) ===
            JSON.stringify(Array.from(decoded.slice(0, original.length)))
