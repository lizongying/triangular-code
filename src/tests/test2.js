    ...
    polyDiv(dividend, divisor) {
        ...
        for (let i = degRem; i >= degDivisor; i--) {
            ...
            for (let j = 0; j <= degDivisor; j++) {
                rem[i - j] = this.sub(
                    rem[i - j],
                    this.mul(normDivisor[degDivisor - j], coef),
                )
            }
        }
...
class ReedSolomon {
    ...
    _generateGeneratorPolynomial() {
        ...
        for (let i = 0; i < 2 * this.t; i++) {
            ...
            for (let j = 0; j < poly.length; j++) {
                ...
                newPoly[j + 1] = this.gf.add(
                    newPoly[j + 1],
                    this.gf.mul(poly[j], root),
                ) // -α^i * poly[j]
            }
            poly = newPoly
        }
    ...
    encode(data) {
        if (data.length > this.k)
            throw new Error(`原始数据长度不能超过 ${this.k} 字节`)
        ...
        // 补零到r长度
        const paddedRemainder = new Array(this.r).fill(0)
        paddedRemainder.set(remainder, this.r - remainder.length)
    ...
    decode(received) {
        if (received.length !== this.n)
            throw new Error(`接收数据长度必须为 ${this.n} 字节`)
...
;(async function testRS() {
    try {
        ...
        const originalData = new Uint8Array([
            0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef, 0x01, 0x23,
        ])
        ...
        console.log(
            '错误位置原始值：',
            errorPositions.map((p) => encodedData[p]),
        )
        console.log(
            '错误位置损坏值：',
            errorPositions.map((p) => corruptedData[p]),
        )
        ...
        console.log(
            '解码修复后数据：',
            Array.from(decodedData.slice(0, originalData.length)),
        )

        // 6. 验证结果
        const isEqual =
            JSON.stringify(Array.from(originalData)) ===
            JSON.stringify(
                Array.from(decodedData.slice(0, originalData.length)),
            )
